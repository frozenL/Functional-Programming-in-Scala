...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    val massX = bodies.foldLeft(0f)((acc, e) => acc + e.mass * e.x) / mass
    val massY = bodies.foldLeft(0f)((acc, e) => acc + e.mass * e.y) / mass
    ...
    def insert(b: Body): Quad =
      if (size > minimumSize) {
        ...
        bodies.foldLeft(Fork(nw, ne, sw, se))((acc, b) => acc.insert(b))
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      matrix(Math.min((b.y / sectorSize).toInt, sectorPrecision - 1) * sectorPrecision + Math.min((b.x / sectorSize).toInt, sectorPrecision - 1)) += b
      this
    }
