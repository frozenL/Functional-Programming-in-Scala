...
object TimeUsage {
  ...
  /** @return The schema of the DataFrame, assuming that the first given column has type String and all the others
    ...
    */
  def dfSchema(columnNames: List[String]): StructType = StructType(columnNames.indices.map(i => {
    if (i == 0) StructField(columnNames(i), StringType, false)
    else StructField(columnNames(i), DoubleType, false)
  }))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    columnNames.aggregate((List[Column](), List[Column](), List[Column]())) ((agg, e) => {
      if(List("t01", "t03", "t11", "t1801", "t1803").exists(s => e.startsWith(s))) {
        (agg._1 :+ new Column(e), agg._2, agg._3)
      } else if (List("t05", "t1805").exists(s => e.startsWith(s))) {
        (agg._1, agg._2 :+ new Column(e), agg._3)
      } else {
        (agg._1, agg._2, agg._3 :+ new Column(e))
      }
    }, (agg1, agg2) => (agg1._1 ++ agg2._1, agg1._2 ++ agg2._2, agg1._3 ++ agg2._3) )
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when($"telfs" >= 1.0 && $"telfs" < 3.0, "working").otherwise("not working").as("working")
