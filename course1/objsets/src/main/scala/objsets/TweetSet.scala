...
//  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet
   ...
   */
    def descendingByRetweet: TweetList = {
      def desAcc(acc: TweetSet): TweetList = {
        if(acc.isEmpty) Nil
        else {
          var mostRet = acc.mostRetweeted
          new Cons(mostRet, desAcc(acc.remove(mostRet)))
        }
      }
      desAcc(this)
    }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    val filtered = (left union right).filter((t: Tweet) => t.retweets > elem.retweets)
...
object GoogleVsApple {
  ...
  def p(list: List[String]) = (tweet: Tweet) => list.exists(e => tweet.text.contains(e))
