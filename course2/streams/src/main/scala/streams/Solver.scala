...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] =
                         neighbors.filterNot(e => explored.contains(e._1))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case _ => {
      ...
      initial.head #:: from(initial.tail ++ newNeighbors, 
        explored ++ newNeighbors.map(e => e._1))
    }
  }
   ...
   * position.
   */
  lazy val solution: List[Move] = if (pathsToGoal.isEmpty) Nil
